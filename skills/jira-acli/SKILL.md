...
| P2 | **쓰기 전 확인 게이트.** 만들거나 바꾸거나 남기는 모든 동작은 (a) 실행할 명령 원문과 (b) 실제로 들어갈 내용을 사용자에게 보여주고, **명시적인 "네"** 를 받은 뒤에만 실행한다. **Jira든 Confluence든 예외 없다** — 작업 항목 만들기·코멘트·수정·**상태 변경**, 그리고 Confluence 블로그 글 작성 전부 해당한다. acli 자체의 확인 프롬프트에 의존하지 않는다. | `references/write.md`, `references/transition.md`, `references/confluence.md` |
