...
- **로그인 명령(`acli jira auth login --web`)은 반드시 사람이 직접 터미널에서 실행해야 합니다.**
  브라우저 승인 뒤 터미널에서 방향키로 사이트를 고르는 단계가 있고, 그 명령이 살아 있는 동안에만
  승인이 완료됩니다. 당신이 Bash로 실행하면 멈추거나, 브라우저에 "성공"이 떠도 로그인이 되지
  않습니다.
